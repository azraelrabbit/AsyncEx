...
namespace Nito.AsyncEx
{
    ...
    public class AsyncSemaphoreSlim : IDisposable
    {
        ...
        private static readonly Task<bool> s_trueTask = new Task<bool>(_ =>true,false,default(CancellationToken),TaskCreationOptions.LongRunning);
        ...
        private readonly static Task<bool> s_falseTask = new Task<bool>(_ => false, false, default(CancellationToken), TaskCreationOptions.LongRunning);
        ...
        private sealed class TaskNode : Task<bool>
        {
            ...
            private static readonly Func<bool> p = () => { return true; };

            internal TaskNode() : base(p) { }

            internal   void ExecuteFromThreadPool(Thread threadPoolThread)
            {
        ...
        public AsyncSemaphoreSlim(int initialCount, int maxCount)
        {
            if (initialCount < 0 || initialCount > maxCount)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(initialCount), initialCount, "initial count wrong");
            }

            //validate input
            if (maxCount <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(maxCount), maxCount, "init max count wrong");
            }
        ...
        //            nameof(timeout), timeout, "wait timeout");
        //    }
        ...
        //    {
        //        throw new ArgumentOutOfRangeException(
        //            nameof(millisecondsTimeout), millisecondsTimeout, "wait timeout");
        //    }
        ...
        public Task<bool> WaitAsync(int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            if (cancellationToken.IsCancellationRequested)
                return TaskEx.FromResult(false);//.FromCanceled<bool>(cancellationToken);
        ...
        private async Task<bool> WaitUntilCountOrTimeoutAsync(TaskNode asyncWaiter, int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            {
                var waitCompleted = TaskEx.WhenAny(asyncWaiter, TaskEx.Delay(millisecondsTimeout, cts.Token));
        ...
        public int Release(int releaseCount)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    nameof(releaseCount), releaseCount, "release count error");
            }
            int returnCount;
            ...
            lock (m_lockObj)
            {
                ...
                if (m_asyncHead != null)
                {
                    ...
                    while (maxAsyncToRelease > 0 && m_asyncHead != null)
                    {
                        ...
                        //var waitCall = new WaitCallback(waiterTask.ConfigureAwait(false).GetAwaiter().GetResult);

                        //ThreadPool.UnsafeQueueUserWorkItem(()=> { }, true);
                    }
                }
        ...
        private void CheckDispose()
        {
            if (m_lockObj == null)
            {
                throw new ObjectDisposedException(null, "asyncSemaphoreSlim disposed ");
            }
        }
        #endregion
    }
